...
passport.use(new LocalStrategy(function verify(username, password, cb) {
    ...
    userdb.get('SELECT * FROM users WHERE username = ?', [ username ], function(err, row)
    {
        if (err) 
        { 
            return cb(err);
...
router.route('/checkuser/:username').get((request, response) => {
    ...
    userdb.get('SELECT username FROM users WHERE username = ? LIMIT 1', [ userid ], (err, row) => {
        if (err)
        {
            return response.status(500).json({ available: false, message: 'Database error' });
        }
        if (row) 
        {
            return response.json({ available: false, message: 'Username already taken' });
        }
...
// Protected pages 
// kubkars is the main page
router.get('/kubkars', (req, res) => {
    if (!req.isAuthenticated()) return res.redirect('/login');
    res.sendFile(path.join(__dirname,'../pages/kubkars.html'));
});

router.get('/monitoring', (req, res) => {
    if (!req.isAuthenticated()) return res.redirect('/login');
    res.sendFile(path.join(__dirname,'../pages/monitoring.html'));
});

router.get('/recording', (req, res) => {
    if (!req.isAuthenticated()) return res.redirect('/login');
    res.sendFile(path.join(__dirname,'../pages/recording.html'));
});

router.get('/setting_up', (req, res) => {
    if (!req.isAuthenticated()) return res.redirect('/login');
    res.sendFile(path.join(__dirname,'../pages/setting_up.html'));
});
...
router.post('/login/password', passport.authenticate('local', {
    successReturnToOrRedirect: '/kubkars',
...
/* GET /signup
 *
 * This route prompts the user to signup.
 *
...
router.post('/signup', function(req, res, next) {
    ...
    crypto.pbkdf2(req.body.password, salt, 310000, 32, 'sha256', function(err, hashedPassword) 
    {
        ...
        userdb.run('INSERT INTO users (username, hashed_password, salt) VALUES (?, ?, ?)', [
          req.body.username,
          hashedPassword,
          salt
        ], function(err)
